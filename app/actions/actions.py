...
from scripts import explanation_scripts, exhibit_scripts
...
class ActionGetArticleMatches(Action):
    ...
    def run(self,
        ...
        matches = []
        if subject:
            matches += explanation_scripts.get_article_matches(subject)
...
class ActionGetExhibitMatchIds(Action):
    ...
    def run(self,
        ...
        alias_matches = []
        if exhibit:
            alias_matches += exhibit_scripts.get_exhibit_alias_matches(exhibit)
        match_ids = []
        for alias in alias_matches:
            match_ids += exhibit_scripts.get_exhibit_ids(alias)
...
class ActionGiveExplanation(Action):
    ...
    def run(self,
        ...
        if matches_available:
            ...
            explanation = explanation_scripts.get_article_text(article_id=next_id, title=next_title)
            if num_tries == 0:
...
class ActionVerifyExhibit(Action):
    ...
    def run(self,
        ...
        if not alias:
            ...
            return []
        if matches_available:
            exhibit_name = exhibit_scripts.get_exhibit_name(match_ids[num_tries])
...
class ActionUtterFaveExhibit(Action):
    ...
    def run(self,
        ...
        exhibit_id, exhibit_name, fun_fact = exhibit_scripts.get_fave_exhibit()
        if fun_fact:
...
class ActionUtterExhibitResponse(Action):
    ...
    def run(
        ...
        exhibit_name = exhibit_scripts.get_exhibit_name(exhibit_id)
        ...
        if not exhibit_id:
            ...
            short_sum, medium_sum, fun_facts = exhibit_scripts.get_about_exhibit(exhibit_id)
            if short_sum:
            ...
            year = exhibit_scripts.get_exhibit_date(exhibit_id)
            if not year:
            ...
            related_ids = exhibit_scripts.get_related_exhibit(exhibit_id)
            if not related_ids:
                msg = f"I'm sorry, but I'm not sure which exhibits are related to {exhibit_name}."
                dispatcher.utter_message(text=msg)
            else:
                related_id = random.choice(related_ids)
                related_name = exhibit_scripts.get_exhibit_name(related_id)
